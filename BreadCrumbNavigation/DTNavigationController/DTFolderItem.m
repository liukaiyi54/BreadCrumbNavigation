...
- (void)setViewWithFolderName:(NSString *)folderName
{
    ...
    if (!self.hideArrowForFirstItem) {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"层级箭头"]];
        [imageView setTag:2008];
        imageView.frame = CGRectMake(10, 18, 4, 7);
...
#pragma mark - setter
- (void)setHideArrowForFirstItem:(BOOL)hideArrowForFirstItem {
    _hideArrowForFirstItem = hideArrowForFirstItem;
    
    if (_hideArrowForFirstItem) {
        UIImageView *arrow = [self viewWithTag:2008];
        arrow.hidden = YES;
    }
}
